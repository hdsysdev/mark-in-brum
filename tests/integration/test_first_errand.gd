...
func _make_floor_and_nav() -> void:
	...
	box.size = Vector3(700.0, 1.0, 700.0)
	shape.shape = box
	shape.position = Vector3(0.0, -0.5, 0.0)
	body.add_child(shape)
	add_child_autofree(body)
	var nav := FlatNavRegion.new()
	nav.size = Vector2(700.0, 700.0)
	nav.cell_size = 8.0
...
func test_chain_completes_from_launch_to_free_roam() -> void:
	...
	# Step 1: enter the real resource-authored New Street marker.
	mark.global_position = director.current_step().trigger_position + Vector3(0.0, 0.9, 0.0)
	await wait_physics_frames(10, "reach New Street marker")
	assert_eq(director.current_step().id, "toilet_closed", "proximity must advance to step two")

	# Step 2: enter the resource-authored blocked-toilet marker.
	mark.global_position = director.current_step().trigger_position + Vector3(0.0, 0.9, 0.0)
	...
	civilian.position = mark.global_position + Vector3(0.0, 0.0, -2.0)
	add_child_autofree(civilian)
	var aim := Node3D.new()
	mark.add_child(aim)
	aim.position = Vector3(0.0, 0.8, 0.0)
	aim.look_at(civilian.global_position + Vector3(0.0, 0.3, 0.0), Vector3.UP)
	...
	# Step 4: enter the resource-authored Victoria Square marker.
	mark.global_position = director.current_step().trigger_position + Vector3(0.0, 0.9, 0.0)
...
func test_director_reports_objective_changes() -> void:
	...
	(director.chain.steps[0] as ErrandStep).trigger_radius = 1000.0  # capture Mark immediately
